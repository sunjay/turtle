...
impl Renderer {
    ...
    pub fn render<'a>(
        ...
        self.renderer
            .replace_dest_framebuffer(DestFramebuffer::full_window(vec2i(
                draw_size.width as i32,
                draw_size.height as i32,
            )));
        ...
        canvas.stroke_rect(pathfinder_geometry::rect::RectF::new(
            vec2f(0.0, 0.0),
            vec2f(1.0, 1.0),
        ));
        ...
        for prim in display_list.iter() {
            match prim {
                &DrawPrim::Line(Line {
                    ...
                    path.move_to(
                        ScreenPoint::from_logical(start, dpi_scale, center, fb_center).into(),
                    );
                    path.line_to(
                        ScreenPoint::from_logical(end, dpi_scale, center, fb_center).into(),
                    );
                ...
                &DrawPrim::Polygon(Polygon {
                    ...
                    path.move_to(
                        ScreenPoint::from_logical(points[0], dpi_scale, center, fb_center).into(),
                    );
