...
async fn serve(
    ...
    let anim_runner = AnimationRunner::new(conn.clone(), app.clone(), display_list.clone(), event_loop.clone());
...
fn dispatch_request(
    ...
    match request {
        ...
        Export(path, format) => handlers::export_drawings(conn, &app.read(), &display_list.lock(), &path, format),

        PollEvent => handlers::poll_event(conn, events_receiver),

        DrawingProp(prop) => handlers::drawing_prop(conn, &app.read(), prop),
        SetDrawingProp(prop_value) => handlers::set_drawing_prop(&mut app.write(), event_loop, prop_value),
        ...
        SetTurtleProp(id, prop_value) => handlers::set_turtle_prop(&mut app.write(), &mut display_list.lock(), event_loop, id, prop_value),
        ResetTurtleProp(id, prop) => handlers::reset_turtle_prop(&mut app.write(), &mut display_list.lock(), event_loop, id, prop),
        ResetTurtle(id) => handlers::reset_turtle(&mut app.write(), &mut display_list.lock(), event_loop, id),
        ...
        BeginFill(id) => handlers::begin_fill(&mut app.write(), &mut display_list.lock(), event_loop, id),
        EndFill(id) => handlers::end_fill(&mut app.write(), id),

        ClearAll => handlers::clear_all(&mut app.write(), &mut display_list.lock(), event_loop, anim_runner),
        ClearTurtle(id) => handlers::clear_turtle(&mut app.write(), &mut display_list.lock(), event_loop, id),
        ...
        DestroyDrawing => handlers::destroy_drawing(event_loop),
    }
}
